...
def create_population(number, cantus_firmus):
    ...
    for i in range(number):
        ...
        for note in cantus_firmus:
            valid_first_beat_range = [
                interval
                for interval in VALID_FIRST_BEAT_INTERVALS
                if (interval + note) < 17
            ]
            valid_third_beat_range = [
                interval
                for interval in VALID_THIRD_BEAT_INTERVALS
                if (interval + note) < 17
            ]
...
class Genome(ga.Genome):
    ...
    def mutate(self, mutation_range, mutation_rate, context):
        ...
        """
        first_beat_mutation_intervals = [
            interval
            for interval in VALID_FIRST_BEAT_INTERVALS
            if interval <= mutation_range
        ]
        third_beat_mutation_intervals = [
            interval
            for interval in VALID_THIRD_BEAT_INTERVALS
            if interval <= mutation_range
        ]
